...
import gitRepositoryProvider, { type GitRepository, GIT_COMMIT_FAIL_INTRUSIVE_TEST } from "../../src/git.js";
import { type Iconfile } from "../../src/icon.js";
import {
	createTestGitRepo,
	deleteTestGitRepo,
	getCurrentCommit,
...
describe("git access functions", () => {
	...
	describe("should include addIconfile which", () => {
		...
		it("should throw an error, but preserve the last consistent git repo state, in case adding an icon file failse",
			async () => {
				const iconInfo: Iconfile = {
					name: "pizza",
					format: "thin-crust",
